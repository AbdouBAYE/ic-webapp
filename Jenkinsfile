...
pipeline {
     ...
     stages {
         stage('Build image') {
             ...
             steps {
                script {
                  sh 'docker build -t ${ID_DOCKER}/$IMAGE_NAME:$IMAGE_TAG -f ./Docker-ressources/Dockerfile ./Docker-ressources'
                }
             }
        }
      ...
      stage('Scan Image with  SNYK') {
            ...
            steps {
                script{
                    ...
                    SCAN_RESULT=$(docker run --rm -it -e SNYK_TOKEN=$SNYK_TOKEN -v /var/run/docker.sock:/var/run/docker.sock -v $(pwd):/app snyk/snyk:docker snyk test --docker $ID_DOCKER/$IMAGE_NAME:$IMAGE_TAG --json ||  if [[ $? -gt "1" ]];then echo -e "Warning, you must see scan result \n" ;  false; elif [[ $? -eq "0" ]]; then   echo "PASS : Nothing to Do"; elif [[ $? -eq "1" ]]; then   echo "Warning, passing with something to do";  else false; fi)
                    echo "Scan ended"
                    '''
                }
            }
